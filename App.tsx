...
const App: React.FC = () => {
  ...
  const handleLogin = (user: UserAccount) => {
    setCurrentUser(user);
    setActiveTab('dashboard'); // Explicitly reset tab to dashboard on login
  ...
  const isLicenseValid = new Date(data.license.expiryDate) > new Date();
  ...
  if (!isLicenseValid && currentUser.role !== 'Admin') {
    return (
      <div className="min-h-screen bg-rose-50 flex items-center justify-center p-6 text-center">
        <div className="max-w-md bg-white p-12 rounded-[3rem] shadow-2xl border border-rose-100">
          <div className="bg-rose-100 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-8 text-rose-600">
            <Lock className="w-10 h-10" />
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter mb-4">License Expired</h1>
          <p className="text-slate-500 font-medium mb-8 leading-relaxed">
            Your access license has expired on <span className="text-rose-600 font-bold">{new Date(data.license.expiryDate).toLocaleDateString()}</span>. 
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F8FAFC] selection:bg-emerald-100 selection:text-emerald-700">
      <Sidebar 
        activeTab={activeTab} 
      ...
      />
      <main className="flex-1 px-4 py-8 md:px-10 lg:px-16 overflow-x-hidden pb-32 md:pb-8">
        ...
        {!isLicenseValid && currentUser.role === 'Admin' && (
          <div className="mb-8 bg-rose-600 text-white px-6 py-4 rounded-2xl flex items-center justify-between shadow-xl animate-pulse">
            ...
            <button 
              ...
              className="bg-white text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest"
            >
              Renew Period
            </button>
          </div>
